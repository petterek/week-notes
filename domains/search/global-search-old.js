 ...
 */
(function () {
    if (customElements.get('global-search')) return;
    ...
    class GlobalSearch extends HTMLElement {
        ...
        connectedCallback() {
            if (!this.service) {
                ...
                console.error('<global-search>:', why);
            ...
            const render = (data, q) => {
                if (!data || data.length === 0) {
                    resultsEl.innerHTML = html`<p style="color:var(--text-muted);font-style:italic">Ingen treff for «${q}»</p>`;
                    return;
                }
                const groups = {};
