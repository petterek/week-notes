 ...
 */
(function () {
    if (customElements.get('week-calendar')) return;
    ...
    class WeekCalendar extends HTMLElement {
        ...
        connectedCallback() {
            if (!this.service) {
                ...
                console.error('<week-calendar>:', why);
        ...
        set workHours(v) { this._workHours = v; this.render(); }
        ...
        // ---- Items API ----
        // Each item: { startDate, endDate, heading, body, type, moveable, id, data }
        ...
        clearItems() { this._items = []; this.render(); }
        removeItem(id) {
        ...
        render() {
            ...
            this._style.textContent = buildCss(HP, HE - HS, dayCount);
            if (!days.length) { this._wrap.innerHTML = '<div style="padding:12px;color: var(--text-subtle)">Ingen dager å vise</div>'; return; }
            const hourCells = [];
            ...
            this._wrap.innerHTML = `<div class="grid"><div class="corner"></div>${dayHeads}<div class="hours">${hourCells.join('')}</div>${dayCols}</div>`;
        ...
        _wireItemEvents() {
            if (!this._wrap) return;
            this._wrap.querySelectorAll('.item').forEach(el => {
        ...
        _updateNowLine() {
            const el = this._wrap && this._wrap.querySelector('.now-line[data-now]');
            if (!el) return;
