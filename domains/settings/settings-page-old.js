 ...
 */
(function () {
    if (customElements.get('settings-page')) return;

    const { html, escapeHtml } = window.WN;
    ...
    class SettingsPage extends HTMLElement {
        ...
        connectedCallback() {
            if (!this.service) {
                ...
                console.error('<settings-page>:', why);
            ...
            if (!this.shadowRoot) {
                ...
                `}`;
                this._railEl = this.shadowRoot.querySelector('.sp-rail');
                this._detailEl = this.shadowRoot.querySelector('.sp-detail');
            }
            this._selected = null;
            this.refresh();
        }
        ...
        async refresh() {
            try {
                ...
                this._railEl.textContent = 'Kunne ikke laste kontekster.';
            }
        }
        ...
        renderRail() {
            ...
            this._railEl.innerHTML = items;
            this._railEl.querySelectorAll('.rail-item').forEach(el => {
        ...
        renderDetail() {
            ...
            if (!c) { this._detailEl.textContent = 'Ingen kontekst valgt.'; return; }
            const s = c.settings || {};
            ...
            this._detailEl.querySelectorAll('[data-wh-on]').forEach(cb => {
            ...
            const swBtn = this._detailEl.querySelector('[data-switch]');
            if (swBtn) swBtn.addEventListener('click', () => this.switchTo(c.id));

            this._detailEl.querySelector('.save').addEventListener('click', () => this.save(c));
        }

        async switchTo(id) {
            const status = this._detailEl.querySelector('[data-status]');
        ...
        collectForm() {
            const f = (k) => {
                const el = this._detailEl.querySelector(`[data-f="${k}"]`);
                return el ? el.value : '';
            };
            const wh = DAY_NAMES.map((_, i) => {
                const row = this._detailEl.querySelector(`[data-wh-row="${i}"]`);
                if (!row) return null;
        ...
        async save(ctx) {
            const status = this._detailEl.querySelector('[data-status]');
            const merged = Object.assign({}, ctx.settings || {}, this.collectForm());
