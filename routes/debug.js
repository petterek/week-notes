...
modal.open();</pre>
            ...
            'date-time-picker': {
                desc: `<p><strong>&lt;date-time-picker&gt;</strong> is a custom calendar popup styled to match week-notes. It renders a Mon-first month grid (Norwegian weekday/month labels), prev/next navigation, an &ldquo;I dag&rdquo; (today) shortcut and Avbryt/OK actions. In <code>datetime</code> mode it also shows hour/minute selects (5-minute step).</p>
                    ...
                    <p><strong>Events.</strong> <code>datetime-selected</code> with detail <code>{ value }</code> when the user clicks OK / double-clicks a day / presses Enter. <code>dateWeekSelected</code> with detail <code>{ value }</code> (where <code>value</code> is the ISO week <code>YYYY-Www</code> of the selected day) when the user presses <code>w</code>. <code>datetime-cancelled</code> when Avbryt or Escape is used. All bubble and are composed.</p>
