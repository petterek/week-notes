...
function setActiveContext(name, opts) {
    ...
    // Drop any stale autosave dotfiles from both the context we just
    // left and the one we're switching to — no editor can be mid-save
    // across a context switch, so anything still on disk is leftover.
    if (prevCtx) try { cleanAutosaveFiles(prevCtx); } catch {}
    try { cleanAutosaveFiles(safe); } catch {}
...
// the editing session — at startup or on context switch we know no
// editor is mid-save, so any remaining ones are stale.
function cleanAutosaveFiles(ctxId) {
    const id = ctxId || getActiveContext();
    if (!id) return 0;
...
function loadMeetingsForContext(ctxId) {
    const safe = safeName(ctxId);
    const dir = path.join(CONTEXTS_DIR, safe, 'meetings');
    let entries;
    ...
    for (const fname of entries) {
        if (!fname.endsWith('.json')) continue;
        try {
            items.push(JSON.parse(fs.readFileSync(path.join(dir, fname), 'utf-8')));
        } catch { /* skip */ }
    }
    if (items.length) return items;
    // Fallback to legacy single-file format
    try {
        const arr = JSON.parse(fs.readFileSync(path.join(CONTEXTS_DIR, safe, 'meetings.json'), 'utf-8'));
...
function getContextThemes(ctxId) {
    ...
    // resolves via getActiveContext). For other contexts we return
...
(function(){
    var ROUTES = {
        ...
        '/calendar/all': '/pages/calendar-all.html'
    };
    var ROUTE_PATTERNS = [
        { re: /^\\/editor\\/[^/]+\\/[^/]+\\.md$/, frag: '/pages/editor.html' },
        { re: /^\\/calendar\\/all\\/\\d{4}-W\\d{2}$/, frag: '/pages/calendar-all.html' },
        { re: /^\\/calendar\\/\\d{4}-W\\d{2}$/, frag: '/pages/calendar.html' },
        { re: /^\\/team\\/[^/]+$/, frag: '/pages/team.html' }
    ];
...
// inline <task-create-full id="tasksPageCreate"> instead of opening the
// modal. Runs in capture phase so it beats the singleton modal listener.
(function(){
    document.addEventListener('task:request-edit', function(ev){
        var form = document.querySelector('task-create-full#tasksPageCreate');
        if (!form) return; // not on tasks page
        if (form.contains(ev.target)) return; // dispatched by the form itself
        ev.stopImmediatePropagation();
        var detail = ev.detail || {};
        var task = detail.task || {};
        var cb = (typeof detail.callback === 'function') ? detail.callback : null;

        // Mark the source row as "editing" so the user can see what
        // they're editing. Look it up across any task-open-list on the page.
        var lists = document.querySelectorAll('task-open-list');
        // Clear any prior editing mark first — only one row at a time.
        ...
        // Scroll the page so the edited row sits just below the sticky
        // form. Use scroll-margin-top on the row + scrollIntoView so the
        // browser handles the offset math regardless of scroll container.
        if (editingRow) {
            ...
            // Wait two frames so the form's edit-mode prefill has reflowed
            // (changes the sticky card's height).
...
(function(){
    ...
    function renderMarkdown(md) {
        // Strip a single wrapping triple-backtick fence (e.g. some LLMs
        // wrap their entire response). Mirrors stripCodeFence() server-side
        // so saved summary files containing the fence still render right.
        ...
        if (!html) {
            html = '<pre style="white-space:pre-wrap;font-family:inherit">'
                + escapeHtmlClient(src) + '</pre>';
        }
        // Expand @mentions into <entity-mention> elements so they render
        // as linked chips (same pattern as server-side linkMentions).
        html = html.replace(/(^|[\\s\\n(\\[>])@([a-zA-ZæøåÆØÅ][a-zA-ZæøåÆØÅ0-9_-]*)/g,
...
<script type="module" src="/components/tasks-page.js"></script>
...
<script type="module" src="/components/all-contexts-calendar.js"></script>
...
<script type="module" src="/components/team-status-page.js"></script>
<script type="module" src="/components/results-page.js"></script>
<script type="module" src="/components/result-create.js"></script>
<script type="module" src="/components/goals-page.js"></script>
<script type="module" src="/components/active-goals.js"></script>
<script type="module" src="/components/create-modal.js"></script>
...
function linkMentions(html, people, companies, teams) {
    ...
    // Meeting reference markers — created from inline {{m:Title @ date time}}.
