...
// Collect every tag referenced by any note's sidecar meta in the *active*
// context, merge them into settings.availableThemes (case-insensitive dedup,
// preserves the casing already saved in settings). Persists settings when
// new tags are discovered. Returns the canonical (merged) tag list.
//
// Cheap: notes meta is already memoized per context. Settings write only
// happens when something actually changes.
function syncActiveContextTagsFromNotes() {
    const ctx = getActiveContext();
    if (!ctx) return [];
    const settings = getContextSettings(ctx) || {};
    const existing = Array.isArray(settings.availableThemes) ? settings.availableThemes.slice() : [];
    const seen = new Map(); // lowerKey → original casing
    ...
    let changed = false;
    let meta;
    try { meta = loadNotesMeta(); } catch { meta = {}; }
    for (const key of Object.keys(meta || {})) {
        ...
        for (const t of tags) {
            ...
            if (!v) continue;
            const k = v.toLowerCase();
            if (!seen.has(k)) { seen.set(k, v); changed = true; }
        }
    }
    const merged = Array.from(seen.values());
    if (changed) {
        try { setContextSettings(ctx, { ...settings, availableThemes: merged }); }
        catch {}
    }
    return merged;
}
...
module.exports = {
  ...
  getContextSettings,
  getContextThemes,
  syncActiveContextTagsFromNotes,
  getCurrentYearWeek,
